...
class SportsAssessmentDemo:
    ...
    def _reset_session(self):
        ...
        if self.rep_counter:
            # Support both AccurateRepCounter.reset_counter and legacy reset
            if hasattr(self.rep_counter, 'reset_counter'):
                self.rep_counter.reset_counter()
            elif hasattr(self.rep_counter, 'reset'):
...
def main():
    ...
    # Non-interactive overrides via env vars (set by API)
    selected_exercise = os.environ.get('SIH_DEMO_EXERCISE')
    selected_mode = os.environ.get('SIH_DEMO_MODE')

    # Get user choice if not provided via env
